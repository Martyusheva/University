...
-- Реализация может быть только такой, т.к. у нас есть произвольная функция, про которую мы можем сказать только то, что она преобразует тип a в тип b (a -> b), но что представляет из себя тип b мы не знаем, а значит не можем (точнее, можем, но только с результатом false) применять к b предикат contains.
